  ...
  if (!product) {
    return (
      <div className="py-16 sm:py-20 text-center px-4">
        <h1 className="text-xl sm:text-2xl font-bold text-text-primary mb-4">Product not found</h1>
    ...
    <>
      ...
      <div className="bg-surface-alt border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 sm:py-3">
          <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-text-secondary overflow-x-auto whitespace-nowrap">
      ...
      {/* Product Detail */}
      <section className="py-6 sm:py-10 md:py-16 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-16">
            {/* Images */}
            <div>
              <div className="bg-surface-alt rounded-xl sm:rounded-2xl overflow-hidden">
                <img
            ...
            <div>
              <AnimatedSection>
                ...
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-text-primary mb-2 sm:mb-3">{product.name}</h1>
                <p className="text-sm sm:text-base md:text-lg text-text-secondary mb-4 sm:mb-6 leading-relaxed">{product.description}</p>
              </AnimatedSection>

              {/* Pricing */}
              <div className="bg-surface-alt rounded-xl sm:rounded-2xl p-4 sm:p-6 mb-4 sm:mb-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    <p className="text-xs sm:text-sm text-text-secondary mb-1">Full Price</p>
                    <p className="text-2xl sm:text-3xl font-bold text-text-primary">{formatPrice(product.price)}</p>
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm text-text-secondary mb-1">Pay Small Small</p>
                    <p className="text-base sm:text-lg font-bold text-primary">{formatPrice(product.deposit)} deposit</p>
                    <p className="text-xs sm:text-sm text-text-secondary">{formatPrice(monthlyPayment)}/month x 6</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4 sm:mb-6">
                <h3 className="text-sm sm:text-base font-bold text-text-primary mb-2 sm:mb-3">What It Powers</h3>
              ...
              {/* Why this product */}
              <div className="mb-4 sm:mb-6">
                <h3 className="text-sm sm:text-base font-bold text-text-primary mb-2 sm:mb-3">Why This Product</h3>
                <ul className="space-y-1.5 sm:space-y-2">
                  {product.whyThisProduct.map((reason) => (
                    <li key={reason} className="flex items-center gap-2 text-text-secondary text-xs sm:text-sm">
              ...
              <div className="mb-5 sm:mb-8">
                <h3 className="text-sm sm:text-base font-bold text-text-primary mb-2 sm:mb-3">What You Get</h3>
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
                  {product.whatYouGet.map((item) => (
                    <li key={item} className="flex items-center gap-2 text-text-secondary text-xs sm:text-sm">
      ...
      {/* Related Products */}
      {related.length > 0 && (
        <section className="py-8 sm:py-12 md:py-16 bg-surface-alt transition-colors duration-300">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-xl sm:text-2xl font-bold text-text-primary mb-6 sm:mb-8">You May Also Like</h2>
